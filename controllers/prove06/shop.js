...
exports.getProducts = (req, res, next) => {
  Product.find()
    .sort('title')
    .then(products => {
      res.render('pages/proveAssignments/prove06/shop/product-list', {
        prods: products,
...
exports.getProduct = (req, res, next) => {
  ...
  Product.findById(prodId) // findById is mongoose method
    .then(product => {
      res.render('pages/proveAssignments/prove06/shop/product-detail', {
        product: product,
...
exports.getIndex = (req, res, next) => {
  Product.find()
    .sort('title')
    .then(products => {
      res.render('pages/proveAssignments/prove06/shop/index', {
        prods: products,
        pageTitle: 'Shop',
        path: '/',
        isAuthenticated: req.session.isLoggedIn,
...
exports.getCart = (req, res, next) => {
  req.user
    ...
    .then(user => {
      ...
      res.render('pages/proveAssignments/prove06/shop/cart', {
        path: '/cart',
        pageTitle: 'Your Cart',
        products: products,
        cartTotal: cartTotal,
...
exports.postOrder = (req, res, next) => {
  ...
  .then(() => {
    res.redirect('/proveAssignments/06/shop/orders');
  })
  .catch(err => {
    const error = new Error(err);
    error.httpStatusCode = 500;
...
exports.getOrders = (req, res, next) => {
  Order.find({ 'user.userId': req.user._id })
    .then(orders => {
      res.render('pages/proveAssignments/prove06/shop/orders', {
        path: '/orders',
